...
{
    public sealed class ArenaFloorGrid : MonoBehaviour
    {
        [SerializeField] private FloorTile tilePrefab;
        ...
        public void RebuildImmediate()
        {
            if (tilePrefab == null)
            {
                Debug.LogWarning("ArenaFloorGrid requires a tile prefab.", this);
                return;
            }
            ...
            for (var y = 0; y < gridSize.y; y++)
            {
                for (var x = 0; x < gridSize.x; x++)
                {
                    ...
                    tile.transform.localPosition = new Vector3(
                        originOffset.x + x * tileSpacing.x,
                        originOffset.y + y * tileSpacing.y,
                        0f);
